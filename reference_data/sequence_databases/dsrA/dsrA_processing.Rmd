...
write.table(metadata,
            file = "wrk_dir/muller_DsrAB_dataset_metadata.tsv",
            sep = '\t',
            quote = FALSE,
            row.names = FALSE)
...
Next I looked at the alignment of the DsrAB sequences to the HMM file. Th

```{r alignment_check}
library(Biostrings)
library(DECIPHER)
aligned_sequences <- readAAStringSet("wrk_dir/muller_DsrAB_aligned_dsrA_clean.afa",
                                     format = "fasta",
                                     nrec = -1L,
                                     skip = 0L,
...
BrowseSeqs(myXStringSet = aligned_sequences_trimmed,
           htmlFile = "wrk_dir/muller_DsrAB_aligned_dsrA_trimmed.html")
```
...
length_vect_filtered <- length_vect[length_vect > 120]
...
writeXStringSet(final_seqs,
                filepath = "wrk_dir/muller_DsrAB_aligned_dsrA_final.afa",
                format = "fasta")
```